...
:: DOS Config ::
@SET $.CD=%CD%
@PUSHD "%~dp0"&:: Force Current Directory
@VERIFY OFF&:: Do Not Verify Written Data
@ECHO OFF&:: Disable Command Debug Outputs
@ECHO=[?25l&:: Disable Cursor
@ECHO=[3 q&:: Change Cursor Style
@COLOR 0F&:: Default Color Rendering
@PROMPT $^>&:: Prompt Style
@CHCP 65001 1>NUL&::Enable UTF-8
...
IF NOT %1.==READY. IF %1.==LAUNCH. (
	...
	)) && (ECHO.[√] Launched!) || (ECHO. [X] Failed to launch due to a critical error)
	EXIT /B 0
)
...
:: ANSI Colors Init ::
FOR /F "USEBACKQ TOKENS=*DELIMS=" %%I IN ("%RAW%\colors.ans") DO SET %%I
